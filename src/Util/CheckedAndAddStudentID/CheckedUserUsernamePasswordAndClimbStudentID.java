package Util.CheckedAndAddStudentID;
...
public class CheckedUserUsernamePasswordAndClimbStudentID {
	static Logger logger = LogManager.getLogger(CheckedUserUsernamePasswordAndClimbStudentID.class.getName());
	...
	public static Map CheckedUserUsernamePasswordAndClimbStudentID(String username, String password){
		...
		try {
			//暂停5S，防止时间间隔过短被封IP
			Thread.sleep(5 * 1000);
			logger.info("自动检查账号密码正确性，账号：" + username + "密码：" + password);
...
//			BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(getMethodForHome.getResponseBodyAsStream()));
